...
val scanSequence = listOf(
    ScanPose(Face.F, "FRONT", "Choose any face as FRONT. Keep the same top edge pointing upward."),
    ...
    ScanPose(Face.U, "UP", "Return to FRONT, then tilt the cube down so the UP face looks at the camera."),
    ScanPose(Face.D, "DOWN", "Return to FRONT, then tilt the cube up so the DOWN face looks at the camera.")
)
