...
struct WrapperPool {
    int count = 0;
    std::vector<std::uint16_t> setups;
    std::vector<std::uint16_t> undos;

    const std::uint16_t* setup(int index) const {
        return setups.data() + static_cast<std::size_t>(index) * kPermSize;
    }

    const std::uint16_t* undo(int index) const {
        return undos.data() + static_cast<std::size_t>(index) * kPermSize;
    }
};
